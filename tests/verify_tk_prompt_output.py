        ...
        with sync_playwright() as playwright:
            ...
            created = page.evaluate(
                ...
                () => {
                  ...
                  return { gallery: gallery.type };
                }
                """
            )
            check("节点注册并创建", created == {"gallery": "DanbooruGallery"}, created)
            page.wait_for_selector(".anima-danbooru-gallery", timeout=15_000)
            # 「TK 相机控制」节点已于 2026-09-12 退役（由 TK 可动素体相机取代），
            # 原先在此处的 6 组相机前端检查（拖拽模式/几何/权重/roll 可视化）随节点一并移除。
