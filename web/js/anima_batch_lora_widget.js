...
(function () {
  ...
  function showBatchDownloadDialog(onDone) {
    ...
    modal.innerHTML = `<h3 style="margin:0 0 8px;font-size:13px;">🔗 从 C 站链接批量下载 LoRA</h3>
      ...
      <input class="bd-token" type="password" value="${(function(){ try { return localStorage.getItem('anima_civitai_token') || ''; } catch(e){ return ''; } })()}" placeholder="C 站 API Key（推荐，比 cookie 省事）：设置页 https://civitai.com/settings/api-keys 生成后复制到这里" style="margin-top:6px;padding:7px 9px;background:#0a0a0c;color:#EDEDEF;border:1px solid rgba(255,255,255,0.08);border-radius:6px;font-size:10px;outline:none;width:100%;box-sizing:border-box;">
