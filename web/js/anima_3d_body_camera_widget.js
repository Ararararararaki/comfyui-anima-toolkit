...
(function () {
  ...
  // 这四个字段对应 BSK 相机算法的四个轴：左右、上下、距离和倾斜角。
  // 轴权重只控制强度，具体输出哪个 tag 仍由相机当前位置决定。
  const PROMPT_WEIGHT_FIELDS = [
    { id: "azimuth.weight", group: "BSK 权重", label: "左右方位", path: ["azimuth"], default: 10, min: 0, max: 10 },
    { id: "elevation.weight", group: "BSK 权重", label: "上下方位", path: ["elevation"], default: 11, min: 0, max: 20 },
    { id: "distance.weight", group: "BSK 权重", label: "距离方位", path: ["distance"], default: 1, min: 0, max: 10 },
    { id: "tilt.weight", group: "BSK 权重", label: "倾斜角", path: ["tilt"], default: 1, min: 0, max: 10 },
  ];
  ...
  function buildPromptPreview(px, py, pz, roll, config) {
    ...
    if (config.distance?.enabled !== false) {
      const key = promptDistanceKey(pz); const item = config.distance?.categories?.[key];
      if (item?.tag && item.enabled !== false) {
        const ranges = { ecu: [0.7, 1], cu: [0.2, 0.7], medium: [-0.2, 0.2], full: [-0.7, -0.2], wide: [-1, -0.7] };
        const [start, end] = ranges[key]; const fraction = ["medium", "full", "wide"].includes(key) ? (end - pz) / (end - start) : (pz - start) / (end - start);
        const distanceWeight = Object.prototype.hasOwnProperty.call(config.distance || {}, "weight") ? finite(config.distance.weight, 1) : Object.prototype.hasOwnProperty.call(item, "weight") ? finite(item.weight, 1) : 1;
        const weight = distanceWeight + clamp(fraction, 0, 1) * finite(config.extra_master, 1) * finite(config.distance?.extra, 0);
        parts.push(...promptWeighted(item.tag, clampWeight(weight)));
      }
    }
  ...
  class BodyCameraUI {
    ...
    build() {
      ...
      root.innerHTML = `
        ...
        <div class="tk-3d-body-camera-section tk-3d-body-camera-camera-section"><div class="tk-3d-body-camera-section-title"><strong>相机参数</strong><span data-camera-readout>实时更新</span></div><div class="tk-3d-body-camera-readouts" aria-live="polite"><span>Yaw <output data-camera-live="yaw">0°</output></span><span>Pitch <output data-camera-live="pitch">0°</output></span><span>Roll <output data-camera-live="roll">0°</output></span><span>距离 <output data-camera-live="distance">5.00</output></span><span>FOV <output data-camera-live="fov">50°</output></span></div><label class="tk-3d-body-camera-range"><span>距离</span><input data-camera="pz" type="range" min="-1" max="1" step="0.01"><output data-camera-output="pz"></output></label><label class="tk-3d-body-camera-range"><span>倾斜角</span><input data-camera="roll" type="range" min="-1" max="1" step="0.01"><output data-camera-output="roll"></output></label><label class="tk-3d-body-camera-range"><span>FOV</span><input data-camera="fov" type="range" min="20" max="100" step="1"><output data-camera-output="fov"></output></label><div class="tk-3d-body-camera-normalized">坐标 X <output data-camera-live="x">0.00</output> · Y <output data-camera-live="y">0.00</output> · Z <output data-camera-live="z">0.00</output></div></div>
        <div class="tk-3d-body-camera-section tk-3d-body-camera-prompt-section"><div class="tk-3d-body-camera-section-title"><strong>提示词参数</strong><span data-prompt-active>BSK 四轴权重</span></div><div class="tk-3d-body-camera-prompt-weight-groups" data-prompt-weights></div><div class="tk-3d-body-camera-prompt-preview"><span>当前输出</span><code data-prompt-preview>正在计算…</code></div></div>
    ...
    _syncPromptControls(config) {
      ...
      const distanceKey = promptDistanceKey(this.state.pz);
      ...
      const active = [
        ...
        `${distanceField.label} ${promptFmtWeight(readNestedWeight(config, distanceField))}（${distanceKey}）`,
      ];
    ...
    _syncControls() {
      ...
      this.root.querySelector('[data-camera-output="pz"]').textContent = `${distance} · ${this.state.pz > 0.35 ? "特写" : this.state.pz < -0.35 ? "全身" : "中景"}`;
