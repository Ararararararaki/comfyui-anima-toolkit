import { esc, showToast, copyText, icon, setBtnIcon } from '../utils'
...
const MAX_PNG_SIZE = 20 * 1024 * 1024 // 20MB 上限（security 修复）

function parsePngFile(file: File): Promise<UploadedPng | null> {
  // 文件大小检查：超大文件直接拒绝（security 修复，与 LocalManager 一致）
...
export function renderPromptFreq() {
  ...
  // 从 Outputs 提取高频词（结果缓存：files/metadataCache 数量不变时直接复用，避免每次切换栏目重算）
  if (state.files.length > 0 && state.metadataCache.size > 0) {
    if (limitEl) _limit = parseInt(limitEl.value) || 50
    const cacheKey = `${state.files.length}-${state.metadataCache.size}-${_limit}`
    if (cacheKey !== _freqCacheKey) {
      _freqCacheKey = cacheKey
      ...
      if (freq.size > 0) {
        ...
        _freqCacheHtml = `<details class="prompt-freq-freq" ${_freqCollapsed ? '' : 'open'}>
  ...
  if (_uploadedPngs.length > 0) {
    ...
    for (const p of _uploadedPngs) {
      ...
      html += `<div class="prompt-freq-png-card">
        ...
        ${p.hasMeta === false ? `<div class="prompt-freq-png-nometa" style="padding:10px;text-align:center;color:var(--text2);font-size:12px;background:var(--bg3);border:1px solid var(--border);border-radius:6px;margin-top:6px;">
          ⚠️ 该 PNG 无元数据，无法提取 Prompt / 工作流<br>
          <small style="color:var(--text3);line-height:1.7">原因：图片经 QQ/微信传输、截图或二次保存后，内嵌的生成数据会被清除。<br>想要别人的工作流，需让对方发送 <b>未压缩的原始 PNG</b>（直接发原图/文件）。</small>
        </div>` : ''}`
      ...
      if (p.loras.length > 0) {
        ...
        html += `<div class="prompt-freq-png-label-bar" style="margin-top:8px">🧩 LoRA（点击复制 ${p.loraTags?.length ? '标签' : '名称'}）</div>
          ...
          <div class="prompt-freq-png-actions">
            <button class="prompt-freq-png-copyall-lora" data-text="${esc(loraList.join(', '))}">${icon('copy', 12)}复制全部 LoRA</button>
          </div>`
      }
...
export function bindPromptFreqEvents() {
  ...
  document.getElementById('promptFreqContent')?.addEventListener('click', async (e) => {
    ...
    if (transAllBtn) {
      ...
      setBtnIcon(transAllBtn, 'spinner', '翻译中…')
