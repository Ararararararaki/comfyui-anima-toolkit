...
import { parseCivitaiModelId } from '../api/civitai'
...
$$('localUrlBtn')?.addEventListener('click', () => {
  ...
  modal.querySelector('.ld-start')?.addEventListener('click', async () => {
    ...
    for (const url of urls) {
      ...
      const modelId = parseCivitaiModelId(url)
      const parsed = vm ? { versionId: vm[1] } : modelId ? { modelId } : null
      if (!parsed) { logEl.textContent += `✗ 无法解析（需 Civitai 模型链接，civitai.com / civitai.red 均可）: ${url.slice(0, 120)}\n`; continue }
