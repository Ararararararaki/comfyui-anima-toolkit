...
## 最近更新（2026-09-21 · 2.20.0）

- **D站 差分组（父子级作品）**：差分作品的卡片右上角显示「父帖 / 子帖」角标，操作条「差分」按钮按 `parent:<根帖id>` **一次取出整组**，「← 返回」还原原搜索词与页码。
- **P站 多页作品折叠**：一个作品**只出一张卡**（角标显示总页数），操作条「全部页」按进该作品全部页、**瞬时返回** —— 不再把 N 页全铺进瀑布流（那会让视口外的页永远选不中）。
- **悬停浮层改为「停留才弹 + 位置定格」**：浮层不再跟随光标 ⇒ 终于**点得到里面的标签**；**P站 也新增了同一套浮窗**（含 pixiv 官方中文翻译）。C站 仍保持只读。
- **画廊搜索历史**：搜索框**聚焦时**（内容为空）就在联想浮层的位置显示「最近搜索」，**点一下即重新搜**；每条可单独删除、可一键清空。按图源分开（D站 tag / C站·P站 关键词互不污染）、每个节点一份，只存本机（不随工作流走）。
- **D站 画廊 Prompt 浮层可交互**：鼠标可以**移进浮层**停留与滚动（不再一离开卡片就消失），**点其中任一标签即按该标签重新搜索**（仅 D站；C站/P站 的浮层保持只读）。
