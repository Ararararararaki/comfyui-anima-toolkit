...
当前发布版本: **2.5.1**。本次修复 Outputs 卡片「复制 LoRA 标签」按钮消失的问题，并新增后端缩略图直出端点（为下一版秒开 + 高清预览打底）。

## 最近更新（2026-09-11 · 2.5.1）

- **Outputs · 复制 LoRA 标签按钮消失修复**：按钮此前会随浏览操作消失且不恢复（需刷新页面才回来）。根因是界面的「内容签名」只看缓存条目数量，而元数据更新不改变数量，导致加载到 LoRA 数据后界面不重建；叠加元数据缓存被覆盖清空后不再补取、部分图片读取偶发长时间卡死。已改为内容版本号驱动重建、覆盖写入时保留已提取结果、加载判据包含提取状态，并加了读取超时与自动重试。纯前端改动，更新后刷新浏览器（Ctrl+Shift+R）即可生效。
- **后端 · 新增缩略图直出端点**：`/anima/thumb`（512/768 两档 WebP 落盘缓存，LANCZOS 高质量缩放，按内容自动失效）。本版前端尚未接入，作为下一版「首次秒开 + 预览图不再糊」改造的第一步；升级无感知，需重启 ComfyUI 生效。
