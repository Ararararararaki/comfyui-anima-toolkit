...
当前发布版本: **2.4.0**。本次更新集中在 LoRA 探索面板（C 站检索与列表交互）与本地 LoRA 扫描性能。

## 最近更新（2026-09-10）

- **LoRA 探索 · C 站检索修复**：NSFW 筛选此前无论怎么选都只返回 SFW（请求里省略 `nsfw` 参数时，C 站 API 会按仅 SFW 处理），现已始终显式传参；新增「C 站线路」设置，默认走官方 `civitai.com`，当前网络不通时可切到镜像站，模型与作者链接随线路变化；「赞比」等本地排序项不再被当作 API 排序参数下发。
- **LoRA 探索 · 列表交互修复**：空状态不再反复闪烁，改为直接说明「C 站返回多少条、被哪一级筛选筛掉」并提供一键清除；点「加载更多」后底部分页栏不再消失；窗口缩放不再触发整屏重建闪烁。
- **LoRA 探索 · 新增能力**：排序新增「📊 赞比（本地排序）」；质量筛选新增「🎯 高质（下载≥250 且 赞比≥5%）」。
- **本地 LoRA 扫描**：补回哈希计算的原生快速路径（文件 ≤256MB 时直接走 `crypto.subtle`，大文件仍分块 + Worker）；取消扫描时给出明确提示。
