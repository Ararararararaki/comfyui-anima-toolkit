...
当前发布版本: **2.5.0**。本次更新集中在本地 LoRA 管理面板的大库性能（切页 / URL 下载弹窗卡顿修复）。

## 最近更新（2026-09-11）

- **本地 LoRA 管理 · 大库性能修复**：大量 LoRA 时，切换到本页面、打开「URL 下载」弹窗、扫描/匹配过程中页面会卡顿到接近无响应（实测 1200 个 LoRA 下每次切页主线程阻塞约 500ms）。现已改为列表分片渲染（首屏 150 张卡片、滚动到底自动补齐，内容与全量一致），扫描/匹配的高频进度更新合并节流，后端分类快照无变化不再重建，全库元数据补齐延迟到激活后进行，网格预览图按实际显示尺寸加载。实测切页阻塞 521ms → 0ms，弹窗 15ms 打开。纯前端改动，更新后刷新浏览器（Ctrl+Shift+R）即可生效。
