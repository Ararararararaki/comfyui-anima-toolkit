...
节点的 `tag_bundle` 和 `natural_language` 都是可选输入。自然语言会沿用 Danbooru Sorter 的语义归入「未归类词」,与其他 11 个分类一起勾选即可一次联合筛选。没有分类包时也可以直接把普通 Prompt/自然语言接入 `natural_language`;默认应用正则/精准排除,并可单独关闭「过滤自然语言」。
