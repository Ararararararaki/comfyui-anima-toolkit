...
当前发布版本: **2.6.0**。Outputs 预览图改由本地后端直出：秒开、滚动即现、不再模糊。

## 最近更新（2026-09-11 · 2.6.0）

- **Outputs · 预览图秒开 + 高清**：卡片图片改由后端 `/anima/thumb` 直出 512px 高质量 WebP（落盘长期复用），浏览器不再逐张读盘、解码原图、缩放、转存。单张从约 1.4MB 原图降为 30KB 级小图，滚动到哪立即加载；预览图不再模糊。后端不可用时自动回退旧管线。**需重启 ComfyUI 生效**。
- **Outputs · 浏览时点击无响应修复**：元数据加载到位后不再每 250ms 整页重建，改为逐卡增量同步 —— 滚动浏览期间点击目录树、切换栏目即时响应（实测点击延迟 6ms、长任务 0 个）。
- **Outputs · 复制 LoRA 标签按钮消失修复（2.5.1）**：详见更新日志。
