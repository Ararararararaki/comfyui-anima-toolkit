...
- **TK D站画廊**:按标签搜索 Danbooru(Novelu&Danbooru 图库)图片,多选输出 IMAGE + Prompt + **metadata_json(结构化元数据:Danbooru ID / 原始标签数组 / rating / score / 收藏数 / 尺寸 / 文件类型 / 视频标记 / 原图 URL / 失败原因)**,下游可直接筛选与复现;内置分级/时间/评分/收藏筛选、双语 tag 预览、下载、入 Prompt 库
