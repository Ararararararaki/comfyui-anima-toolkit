...
- **多源画廊（D站 / C站 / P站）**：`TK D站画廊` 节点新增**图源下拉**，同一个节点切换三个图源 —— 后端新增协议层 `anima_gallery_sources.py`（统一 item schema、图源注册表、适配器容错加载）与 `anima_gallery_civitai.py` / `anima_gallery_pixiv.py` 两个适配器；前端由后端声明的 **capabilities 驱动控件显隐**，不再出现"点了没反应的开关"。
- **C站（Civitai）图源**：cursor 分页 + 排序 / NSFW 档位 / 时间窗 / 作者 / 模型浏览，带 `withMeta=true` 取回完整 prompt 与采样参数。⚠️ 两个实测限制：**图片端点不支持关键词检索**（上游忽略全部关键词参数，本插件退化为"只在已取回的当页内本地过滤"并在界面标注）；**API key 对取图没有任何影响**，它的真实用途是账号校验。
- **P站（Pixiv）图源**：OAuth 2.0 + PKCE 授权（设置面板里一键取链接、粘贴 code），支持关键词搜索；取图自动附加 `Referer` 以通过 `i.pximg.net` 防盗链。P站**不提供 prompt** —— 它的定位是**素材**：选中后从 `images` 输出，可直接送 WD14 反推。
- **图源密钥面板**：设置弹窗新增「图源密钥」一节 —— C站 API key 可填 / 测试 / 清除（**只回显掩码**），P站显示授权状态与「去授权」入口；两个密钥分开落盘（`data/` 下各自文件，不进仓库、不随包发布）。
- **画廊视觉重做**（按"现代、平滑质感、去 AI 感、**不发光**"）：删掉选中卡片的 22px 紫色光晕、工具条与菜单的彩色 glow 与 inset 高光；浮层阴影统一为中性黑，圆角（6/8px）与过渡时长（150ms）统一；颜色全部桥接宿主主题变量。
- **画廊交互修复**：① **操作条误触** —— 卡片悬浮操作条在视觉隐藏（`opacity:0`）时**未禁用命中**，卡片顶部一整条隐形按钮在吃点击（"按钮太大、老误触"的真因），现改为隐藏即 `pointer-events:none`，并把操作条移到卡片底部 caption 行；② 悬停 Prompt 浮层改为**双向翻转**（右→左、下→上），内容异步加载完会重算位置；③ **节点自动收缩**不再与滚动条/列数变化互相触发的正反馈（表现为节点慢慢变小），只在内容不到 60% 可视高时才收 —— 即「图片适配节点」而不是「节点适配图片」；④ 纵向拉大节点会**补一页并追加**，不再"拉大了不加载"。
- **Prompt Cards 联想键盘**：`Tab` 接受当前候选、`Enter` **还给换行**（仅在按过 ↑/↓ 之后才确认所选）、`Shift+Enter` 永远换行、**输入法组字期间不拦截按键**（此前中文上屏会被误判成选词）。

## 历史更新（2026-09-13 · 2.11.0）
...
- **性能 · 画布拖动不再掉帧**：① 卡片区改为渲染窗口化（首屏 60 张 + 滚动补渲染），卡片 DOM 由 ~1 万降到 ~600；② 消除 12 个整宽遗留 `<canvas>`（10.4MB → 0.8MB）；③ 新增节点级渲染裁剪模块（视口外节点跳过渲染 + 图层提升），实测每帧 `Layerize` −44%、平移帧时最高 −48%。工作流图容器 DOM 21001 → 12063。不想用时控制台执行 `localStorage.setItem('tk_canvas_perf','0')` 后刷新即可关闭。
...
- **TK Danbooru Tag Getter**：`natural_language`（前端显示「统一 Prompt」）可直接接普通 Prompt 或 Packer `ALL_TAGS`，自动识别已知 Tag 并分类；每一类可独立调整 0.05–2.0 权重，1.0 保持原样；旧工作流恢复出的 0 权重会自动修正为 1.0；支持“保留自然语言”开关；`tag_bundle` 仍兼容旧的分类包接法，正则/精准排除可单独作用于自然语言。**面板底部新增「过滤诊断」栏**：跑一次图后自动显示本次**保留了哪些词**、**哪些词因为什么被丢掉**（类别关闭 / `tag_blacklist` 命中 / 正则命中 / 未归类，各自分组），**点类别名可直接开/关该类并就地重算** —— 关错类别时不用再猜。⚠️ 注意：旧 12 类**全部关闭**时不会启用自动分类（整段原样输出，等于不过滤），底栏会显式警告。
...
- `TK/Danbooru`: **TK D站画廊（多源画廊）**。一个节点三个图源（D站 / C站 / P站），按标签或关键词搜索、筛选、下载并输出图片及元数据。
...
- TK D站画廊(多源画廊):按标签搜索 Danbooru(Novelu&Danbooru 图库)图片,多选输出 IMAGE + Prompt + metadata_json(结构化元数据:Danbooru ID / 原始标签数组 / Prompt 分组 / 输出设置 / rating / score / 收藏数 / 尺寸 / 文件类型 / 视频标记 / 原图 URL / 失败原因),下游可直接筛选与复现;内置分级/时间/评分/收藏筛选、Prompt 类别/格式控制、双语 tag 预览、下载、入 Prompt 库
  ...
  - **多源画廊(2.14.0)**:节点顶部新增**图源下拉**,同一个节点在 **D站(Danbooru)/ C站(Civitai)/ P站(Pixiv)** 之间切换;界面控件由后端声明的能力(capabilities)驱动显隐,不适用于当前图源的开关不会出现。**C站**支持排序 / NSFW 档位 / 时间窗 / 作者 / 模型浏览,并取回完整 Prompt 与采样参数(⚠️ 实测两条:上游图片端点**不支持关键词检索**,关键词只在已取回的那页内本地过滤,界面已标注;API key 只用于账号校验,**对取图没有任何影响**)。**P站**走 OAuth 2.0 + PKCE 授权后可关键词搜索,取图自动附加 `Referer` 以通过 `i.pximg.net` 防盗链;P站**不提供 Prompt**,定位是**素材** —— 选中后从 `images` 输出,可直接送 WD14 反推。两个密钥分开存放在设置弹窗的「图源密钥」里(只回显掩码)。
...
完整版本记录见 [CHANGELOG.md](CHANGELOG.md)（当前 **2.14.0**）。下面三条是很早的 **2.3.0** 记录，保留备查:
